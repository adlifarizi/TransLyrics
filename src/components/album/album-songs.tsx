...
// Komponen utama
export default async function AlbumSongs({ id }: { id: string }) {
    if (!id) throw new Error("Can't find the album songs you are looking for");

    try {
        // Delay fetch simulation
        // await new Promise(resolve => setTimeout(resolve, 3000));

        const res = await fetch(`${backendApiUrl}/songs?album_id=${id}&sort_by=album_order&direction=asc`, {
            cache: "no-store", // No cache for fresh results
        });

        if (!res.ok) {
            throw new Error(`Failed to fetch album songs: ${res.status} ${res.statusText}`);
        }
        ...
        if (!songs) {
            throw new Error("Can't find the album songs you are looking for");
        }
